...
import com.duplicateremover07.app.databinding.ActivityOthersScanBinding
...
class OthersScanActivity : BaseActivity() {
    ...
    private fun startScanning() {
        ...
        binding.fileCountText.text = getString(R.string.all_scanned, 0)
    ...
    private suspend fun scanOthers() {
        ...
        contentResolver.query(
            ...
            while (cursor.moveToNext()) {
                ...
                if (size > 0 && File(path).exists() && !isSystemFile) {
                    allFiles.add(MediaFile(id, path, name, size, 0, mimeType))
                }
                ...
                withContext(Dispatchers.Main) {
                    ...
                    binding.fileCountText.text = getString(R.string.all_scanned, current)
                }
    ...
    private suspend fun scanFolderRecursive(
        ...
        for (child in children) {
            ...
            if (scanned % 50 == 0) {
                withContext(Dispatchers.Main) {
                    binding.fileCountText.text = getString(R.string.all_scanned, scanned)
                }
                delay(1)
            }
        }
    ...
    private fun updateBottomBarStats() {
        ...
        binding.totalSizeText.text = getString(R.string.total_size, StorageUtils.formatSize(totalSize))
    }
    ...
    private fun showDeleteConfirmation(files: List<MediaFile>) {
        ...
        dialogView.findViewById<android.widget.TextView>(R.id.dialogMessage).text = 
            getString(R.string.delete_dialog_message, files.size, StorageUtils.formatSize(totalSize))
    ...
    private fun deleteFiles(files: List<MediaFile>) {
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                val nonMediaFiles = files.filter { it !in mediaFiles }

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R && mediaFiles.isNotEmpty()) {
                    val uris = mediaFiles.map { 
                        val baseUri = when {
                            it.mimeType.lowercase().startsWith("image/") -> MediaStore.Images.Media.EXTERNAL_CONTENT_URI
                            it.mimeType.lowercase().startsWith("video/") -> MediaStore.Video.Media.EXTERNAL_CONTENT_URI
                ...
                // For non-media files or older Android versions
                for ((index, file) in files.withIndex()) {
                    try {
                        val f = File(file.path)
                        if (f.exists() && f.delete()) {
                            // Also try to remove from MediaStore if it exists there
                        ...
                        withContext(Dispatchers.Main) {
                            binding.scanProgressCircle.setProgress(progress)
                            binding.fileCountText.text = "Deleted ${index + 1}/${files.size}"
                        }
                    } catch (e: Exception) {
